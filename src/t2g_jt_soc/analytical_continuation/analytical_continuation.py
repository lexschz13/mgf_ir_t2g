...
def fermion_continuation(fl: NDArray[RealScalar], irb: FiniteTempBasis, alpha: RealScalar = 10**-1.1,
                         axis: int = -1, guess: None | NDArray[RealScalar] = None, solver: str = "lsql2") -> NDArray[float]:
    """
    Analytical continuation for fermionic functions in ir-basis.

    Parameters
    ----------
    fl : NDArray[RealScalar]
...
def boson_continuation(fl: NDArray[RealScalar], irb: FiniteTempBasis, alpha: RealScalar = 10**-1.1,
                         axis: int = -1, guess: None | NDArray[RealScalar] = None, solver: str = "lsql2") -> NDArray[float]:
    """
...
def hilbert_ir(fl: NDArray[RealScalar], irb: FiniteTempBasis, eta: float = 1e-8, axis: int = -1) -> NDArray[float]:
    """
    ...
    """
    if abs(eta) > 1e-5:
        warn("Too big integral regularizer paramweter can induce numerical errors.", UserWarning)
    fl = axis_push(fl)
    integrand = lambda w,x: np.sum(fl*irb.v(x)) * irb.v(w) * __quasi_inv(w-x, eta)
